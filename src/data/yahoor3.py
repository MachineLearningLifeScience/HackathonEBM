...
class YahooR3(Dataset):
    def __init__(self, root, download=True, which="mnar_train", prop_debias=0.6, prop_val=0.2, prop_test=0.2):
        ...
        if download:
            api = KaggleApi()
            api.authenticate()
            api.dataset_download_files('limitiao/yahoor3', path=self.root, unzip=True)
    ...
    def check_folder_consistency(self):
        ...
        assert os.path.exists(self.root), "Dataset root folder does not exist."
        assert os.path.exists(os.path.join(self.root, 'random.txt')), " MNAR Train data file is missing."
        assert os.path.exists(os.path.join(self.root, 'sampling_data.txt')), "MCAR Test data file is missing."
        assert os.path.exists(os.path.join(self.root, 'user.txt')), "User data file is missing."



    def setup_data(self):
        path_random = os.path.join(self.root, 'user.txt')
        path_sampling = os.path.join(self.root, 'random.txt')
        self.train_df = pd.read_csv(path_random, header=None, names=['uid','iid','rating']) # No header in the random data
        self.test_df = pd.read_csv(path_sampling,  header=None, names=['uid','iid','rating']) # Header in the sampling data
    ...
    def create_data_matrix(self):
        ...
        n_users_train = self.train_df['uid'].nunique()
        n_users_test = self.test_df['uid'].nunique()
        print("We found {} users in the training set.".format(n_users_train))
        print("We found {} users in the test set.".format(n_users_test))
        n_items = np.union1d(self.train_df['iid'].unique(), self.test_df['iid'].unique()).shape[0]
        print("We found {} items in the training set.".format(n_items))
        assert n_items == 1000, "Number of items is incorrect."
        ...
        for row in self.train_df.itertuples():
            # mnar_matrix[row.uid, row.iid] = max(row.rating -1, 0)
            mnar_matrix[row.uid, row.iid] = row.rating-1
            mnar_mask_matrix[row.uid, row.iid] = 1.0 if row.rating > 0 else 0.0
        ...
        for row in self.test_df.itertuples():
            mcar_matrix[row.uid, row.iid] = row.rating-1
            mcar_mask_matrix[row.uid, row.iid] = 1.0 if row.rating > 0 else 0.0
...
if __name__ == "__main__":
    dataset = YahooR3(root="/scratch/hhjs/data/yahoor3", download=False, which="mnar_train")
